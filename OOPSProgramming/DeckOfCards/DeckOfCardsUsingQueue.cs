...
namespace OOPSProgramming.DeckOfCards
{
    ...
    /// <summary>
    /// have properties of Deck of cards
    /// </summary>
    public class DeckOfCardUsingQueue
    {
        ...
        public void InitializeDeckOfCardsUsingQueue()
        {
            for (int i = 1; i <= 4; i++)
            {
                this.Playerlist1.Add(i);
            }

            foreach (int element in this.Playerlist1)
            {
                this.list = this.queue.Enqueue(this.Playerlist1, element);
            }

            this.list = this.queue.Dequeue(this.list, this.Playerlist1);

            this.Playerlist1 = this.list.AddPlayer(this.list);

            ////loops 1 time for all 4 players.
            foreach (int playerNumber in this.Playerlist1)
            {
